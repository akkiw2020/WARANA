...
function FruitsMix() {
  ...
  const [systemProducts, setSystemProducts] = useState([
    { id: "s17", name: "MixFruitsJam", price: 20, image: "MixFruits.png", isOutOfStock: false },
  ]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      // 1. Get products that explicitly have the "Fruits Mix" category
      const categoryDbProducts = allProducts.filter(p => 
        p.category.toLowerCase() === "fruits mix" || 
        p.category.toLowerCase() === "fruitsmix" ||
        p.category.toLowerCase() === "jam" ||
        p.category.toLowerCase() === "mix fruit jam"
      );
      ...
      console.error("Error fetching fruits mix:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ padding: "40px", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      <h2 style={{ textAlign: "center", color: "#333" }}>Available Fruits Mix</h2>
